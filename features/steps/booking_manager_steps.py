...
@when('I create a booking for flight "{flight_id}" with passenger "{passenger_name}" and {num_seats:d} seat(s)')
...
def step_impl(context, flight_id):
    assert context.current_booking["flight_id"] == flight_id, f"Expected flight ID {flight_id}, but got {context.current_booking['flight_id']}"

@then('the booking should have passenger name "{passenger_name}"')
def step_impl(context, passenger_name):
    assert context.current_booking["passenger_name"] == passenger_name, f"Expected passenger name {passenger_name}, but got {context.current_booking['passenger_name']}"

@then('the booking should have {num_seats:d} seats')
def step_impl(context, num_seats):
    assert context.current_booking["num_seats"] == num_seats, f"Expected {num_seats} seats, but got {context.current_booking['num_seats']}"

@when('I cancel the booking')
def step_impl(context):
    context.cancel_result = context.booking_manager.cancel_booking(context.current_booking_id)

@when('I cancel the first booking')
def step_impl(context):
    if context.booking_ids:
        context.cancel_result = context.booking_manager.cancel_booking(context.booking_ids[0])
    else:
        assert False, "No bookings to cancel"

@then('the booking should be cancelled successfully')
def step_impl(context):
    assert context.cancel_result is True, "Booking was not cancelled successfully"

@then('the list of all bookings should contain {count:d} booking(s)')
def step_impl(context, count):
    all_bookings = context.booking_manager.list_bookings()
    assert len(all_bookings) == count, f"Expected {count} bookings, but found {len(all_bookings)}"

@then('the list of "{status}" bookings should contain {count:d} booking(s)')
def step_impl(context, status, count):
    filtered_bookings = context.booking_manager.list_bookings(status=status)
    assert len(filtered_bookings) == count, f"Expected {count} {status} bookings, but found {len(filtered_bookings)}"
